...
@cli.command("describe_simple", help="Describe images in a CSV. Image must be a url in a CSV column called 'Image'")
@click.option(
    "--model",
    "-m",
    default="claude-3-haiku-20240307",
...
@click.option(
    ...
    "-f",
    default="title",
    help="fields to use for metadata separated by |"
)
@click.option(
    "--image_location",
    "-i",
    default="Medium Image",
    help="The field with the image in the CSV",
)
def describe_simple(model: str, output: str, csv: str, start: str, end: str, fields:str, image_location: str) -> None:
            ...
            if i <= total_images_to_get:
                try:
                    ...
                    for field in fields_that_matter:
                        if row[field] != "":
                            metadata_elements.append(f"{field}: {row[field]}")
                    image = Image(image_uri_or_path=row[image_location])
                    y = ClaudeRequest(
                        model=model,
                        key=os.getenv("CLAUDE_API"),
                        prompt=f'Give this image a title and a description based on its contents. Respond with the message '
                    ...
                    if type(y.text) == str:
                        ...
                        with open(f"{output}/{row[image_location].split('/')[-1].split('.')[0]}.json", "w") as f:
                            json.dump(y.text, f, indent=4)
                    print(f"Done. Total cost was: {y.cost.get('total')}. Results written to {output}/{row[image_location].split('/')[-1].split('.')[0]}")
                    i += 1
                except httpx.ReadTimeout as e:
                    print(f"Read timed out: {row[image_location]}\n")
                    i += 1
